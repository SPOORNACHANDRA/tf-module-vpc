resource "aws_vpc" "main" {
  #we are consuming the variables here
  cidr_block = var.cidr
  tags       = merge(local.tags, { name = "${var.env}-vpc" })
}

module "subnets" {
  # we have subnets in that subnets in that subnet we have multiple subnets in that subnets we have multiple values
  ...
  source   = "./subnets"
  subnets  = each.value
  # this we created becoz we have multiple subnets in that subnets we have multiple sub-subnets  we are sending the info
...
resource "aws_nat_gateway" "ngw" {
  #how many ngw i want how many public subnets
